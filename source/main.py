...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown(
            "- [README.md](https://github.com/xhdlphzr/Pathfinder/blob/main/README.md)"
        )
        st.markdown(
            "- [MIT LICENSE](https://github.com/xhdlphzr/Pathfinder/blob/main/LICENSE)"
        )
    ...
    def home_page():
        ...
        with st.expander("🗺️ 点击查看北京地铁线路图", expanded=False):
            try:
                ...
                st.error(
                    "地铁线路图文件未找到, 请确认beijing_subway.png文件在data目录下"
                )
        ...
        with time_col1:
            time_input = st.text_input(
                "开始时间 (格式: HH:MM, 默认为当前时间):",
                value=datetime.now(pytz.timezone("Asia/Shanghai")).strftime("%H:%M"),
            )
        ...
        if selected_strategy in st.session_state.strategy_config:
            ...
            default_off_peak = st.session_state.strategy_config[selected_strategy][
                "off_peak"
            ]
        else:
            default_peak = 10
        ...
        if selected_strategy != "自定义":
            ...
            st.caption(
                f"当前方案「{selected_strategy}」: 高峰期 {current_config['peak']}分钟, 平峰期 {current_config['off_peak']}分钟"
            )
        else:
        ...
        if st.button("开始路径规划", key="a_star", type="primary"):
            ...
            if start_name not in id_map:
                if same(start_name):
                    ...
                    st.warning(
                        f"起始站点 '{start_name}' 不存在! 您是否想输入 '{suggestion}' ?"
                    )
                    return
            ...
            if end_name not in id_map:
                if same(end_name):
                    ...
                    st.warning(
                        f"终点站点 '{end_name}' 不存在! 您是否想输入 '{suggestion}' ?"
                    )
                    return
            ...
            with st.spinner("正在计算最优路径..."):
                try:
                    ...
                    with col1:
                        st.metric(
                            "预计到达时间",
                            f"{((new_time // 60) % 24):02d}:{new_time % 60:02d}",
                        )
                    with col2:
