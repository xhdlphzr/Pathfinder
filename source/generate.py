    ...
    for i in line:
        for station_id, station_info in line[i].items():
            ...
            if name_prev is not None:
                graph[id_map[name_now]][id_map[name_prev]] = round(
                    distance(
                        (
                            station[str(id_map[name_now])]["纬度"],
                            station[str(id_map[name_now])]["经度"],
                        ),
                        (
                            station[str(id_map[name_prev])]["纬度"],
                            station[str(id_map[name_prev])]["经度"],
                        ),
                    ).m
                )
                graph[id_map[name_prev]][id_map[name_now]] = graph[id_map[name_now]][
                    id_map[name_prev]
                ]

            if name_next is not None:
                graph[id_map[name_now]][id_map[name_next]] = round(
                    distance(
                        (
                            station[str(id_map[name_now])]["纬度"],
                            station[str(id_map[name_now])]["经度"],
                        ...
                        (
                            station[str(id_map[name_next])]["纬度"],
                            station[str(id_map[name_next])]["经度"],
                        ),
                    ).m
                )
                graph[id_map[name_next]][id_map[name_now]] = graph[id_map[name_now]][
                    id_map[name_next]
                ]
